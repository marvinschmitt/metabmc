...
test_that("brms object is made given formula, prior, et cetera", {
  ...
  brms_arg_list_1 <- list(formula = f1, data = dat, family = fam1, prior = prior, backend = "mock", mock_fit = "1", rename = FALSE)
  brms_arg_list_2 <- list(formula = f2, data = dat, family = fam2, prior = prior, backend = "mock", mock_fit = "2", rename = FALSE)
