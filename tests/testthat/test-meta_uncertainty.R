test_that("Fit works given pre-fitted brms object", {
  ...
  mockery::stub(meta_uncertainty, "sample_true_model", pmp_sim_1)
  mockery::stub(meta_uncertainty, "simulate_data", simulated_data_matrix)
  ...
  mockery::stub(meta_uncertainty, "post_prob_from_sim", pmp_sim_2)
  mockery::stub(meta_uncertainty, "get_meta_model_posteriors", "mock_meta_model_posterior")
  mockery::stub(meta_uncertainty, "extract_meta_model_param", "mock_meta_model_parameter")
  mockery::stub(meta_uncertainty, "brms::do_call", c(0.2, 0.2, 0.6))
  mockery::stub(meta_uncertainty, "create_mixture_function", "mock_mixture_function")
  ...
  expect_equal(meta_uncertainty(f1, f2, f2), expected_out)
})
...
test_that("Fit works given formula, family, data, et cetra", {
  ...
  expected_out <- list(pmp_sim = pmp_sim_2, simulated_data_matrix = simulated_data_matrix, meta_model_param = "mock_meta_model_parameter", pmp_obs = c(0.2, 0.2, 0.6), mixture_function = "mock_mixture_function")
  class(expected_out) <- "meta_uncertainty_fit"
  out <- meta_uncertainty(formula_list = list(f1, f2, f2), prior_list = list(prior, prior, prior), family_list = list(fam1, fam2, fam2), data=dat, brms_arg_list = list(list(backend = "mock", mock_fit = "1", rename = FALSE), list(backend = "mock", mock_fit = "2", rename = FALSE), list(backend = "mock", mock_fit = "3", rename = FALSE)))
