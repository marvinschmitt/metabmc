...
output:
  rmarkdown::html_vignette:
    toc: yes
vignette: >
  %\VignetteIndexEntry{Estimating and visualizing posterior distribution of model probability with metabmc}
...
Given the "true model" $M^*$, we can readily simulate synthetic data $y^{s}$ because Bayesian models are generative by definition:

$$
\begin{aligned}
M^* &\sim p(M) \\
\theta^s & \sim p(\theta\mid M^*)\\
y^s & \sim p(y\mid \theta^s, M^*)
\end{aligned}
$$

For this simulated data set $y^s$, we fit each model $M_1, \ldots, M_J$ to obtain a posterior $p(\theta\mid y^s, M_j)$ and ultimately compute the posterior model probabilities $p(M_1\mid y^s), p(M_2\mid y^s), p(M_3\mid y^s)$. In accordance with the notation from the Meta-Uncertainty paper, we abbreviate this as:

$$\mathbf{\overset{s}{\pi}} := (p(M_1 \mid \overset{s}{y}),\ p(M_2 \mid \overset{s}{y}),\ p(M_3 \mid \overset{s}{y}))$$
We can repeat this process (sample true model $M^*$, sample parameters $\theta^s$, simulate data $y^s$, compute PMPs $\overset{s}{\pi}$) multiple times to get a whole sampling distribution of posterior model probabilities. 
